...
import { 
  ...
  CreditCard, LogOut, Sparkles 
} from 'lucide-react'
import Navbar from '../components/Navbar'
import { apiFetch, clearSession } from '../../utils/api-client'
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    void (async () => {
      try {
        const result = await apiFetch('/settings/account')
        setUserName(result.data.fullName || 'Recruiter')
        setEmail(result.data.email || 'recruiter@acme.corp')
      } catch {
  ...
  const saveAccount = async () => {
    try {
      ...
      localStorage.setItem('rankr_user_name', result.data.fullName)
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#f0f5fa]">
      ...
      <main className="max-w-250 mx-auto px-4 sm:px-6 pt-28 sm:pt-36 pb-20">
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1 space-y-8">
            ...
            {/* Profile Section */}
            <div className="bg-white rounded-4xl p-6 sm:p-10 shadow-sm border border-[#e2eaf2]/60">
              <h2 className="text-[#070707] text-2xl font-extrabold tracking-tight mb-8">Account Profile</h2>
              
              <div className="space-y-8">
                <div className="flex items-center gap-6">
                  <div className="w-20 h-20 rounded-3xl bg-linear-to-br from-[#2a85ff] to-[#6eb3ff] flex items-center justify-center text-white text-2xl font-black">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label htmlFor='username' className="text-[11px] font-bold text-[#8a9ab0] uppercase tracking-wider px-1">Display Name</label>
                    <input 
                      id='username'
                      value={userName}
                  ...
                  <div className="space-y-2">
                    <label htmlFor='email' className="text-[11px] font-bold text-[#8a9ab0] uppercase tracking-wider px-1">Email Address</label>
                    <input
                      id='email'
                      value={email}
                ...
                <div className="pt-4">
                  <button onClick={() => { void saveAccount() }} className="px-8 py-3.5 rounded-2xl bg-[#070707] text-white text-sm font-bold hover:bg-[#202020] transition-all shadow-xl">
            ...
            {/* Plan Section */}
            <div className="bg-[#070707] rounded-4xl p-6 sm:p-10 shadow-2xl relative overflow-hidden">

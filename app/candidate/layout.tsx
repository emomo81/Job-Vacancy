...
export default function CandidateLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={`${poppins.className} min-h-screen bg-[#f0f5fa] flex flex-col md:flex-row`}>
      ...
      {/* ── SIDEBAR ── */}
      <aside className={`fixed md:sticky top-0 left-0 w-70 bg-[#070707] border-r border-white/5 flex flex-col h-screen z-50 transition-transform duration-300 md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        
        {/* Brand */}
        <div className="px-6 h-20 flex items-center justify-between border-b border-white/5">
          <Link href="/" className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-[#2a85ff] flex items-center justify-center shrink-0">
        ...
        <div className="px-6 py-8">
          <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-linear-to-br from-[#2a85ff] to-[#6eb3ff] flex items-center justify-center shrink-0">
                <span className="text-white text-xs font-bold" suppressHydrationWarning>
                  {userName.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)}
                </span>
              </div>
              <div className="min-w-0">
                <p className="text-white text-sm font-bold truncate" suppressHydrationWarning>{userName}</p>
        ...
        <div className="p-4 mt-auto border-t border-white/5">
          <Link
            ...
            className="flex items-center gap-3 px-5 py-3.5 rounded-xl text-white/45 hover:text-white/80 hover:bg-white/5 transition-all group"
          >
            <LogOut size={18} />
      ...
      <div className="flex-1 flex flex-col min-w-0 w-full">
        ...
        <header className="h-20 bg-white border-b border-[#e2eaf2] flex items-center justify-between px-4 sm:px-10 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            <Link href="/candidate/notifications" className="relative hidden sm:block text-[#5a6a7a] hover:text-[#2a85ff] transition-colors cursor-pointer" title="Notifications" aria-label="Notifications">
              <Bell size={20} />
            ...
            <Link href="/candidate/profile" className="flex items-center gap-2 sm:gap-3 group">
              <div className="text-right flex flex-col">
                <span className="hidden sm:block text-sm font-bold text-[#070707]" suppressHydrationWarning>{userName}</span>
                <span className="hidden sm:block text-[10px] text-[#8a9ab0] font-semibold uppercase tracking-wider" suppressHydrationWarning>{completion}% Profile Complete</span>
              </div>
              <div className="w-10 h-10 rounded-full border-2 border-[#e2eaf2] p-0.5 group-hover:border-[#2a85ff] transition-all shrink-0">
                <div className="w-full h-full rounded-full bg-linear-to-br from-[#2a85ff] to-[#6eb3ff] flex items-center justify-center">
                  <span className="text-white text-[10px] font-bold" suppressHydrationWarning>
                    {userName.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)}
                  </span>
                </div>
              </div>

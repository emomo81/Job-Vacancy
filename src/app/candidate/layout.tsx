...
export default function CandidateLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={`${poppins.className} min-h-screen bg-[#f0f5fa] flex`}>
      
      {/* ── SIDEBAR ── */}
      <aside className="w-[280px] bg-[#070707] border-r border-white/5 flex flex-col sticky top-0 h-screen z-50">
        
        {/* Brand */}
        <div className="px-8 h-20 flex items-center border-b border-white/5">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Header */}
        <header className="h-20 bg-white border-b border-[#e2eaf2] flex items-center justify-between px-10 sticky top-0 z-40">
          <div className="flex items-center gap-2">
            <span className="text-[#8a9ab0] text-sm font-medium">Candidate Dashboard</span>
            <span className="text-[#e2eaf2] text-sm">/</span>
            <span className="text-[#070707] text-sm font-bold">
              {CANDIDATE_NAV.find(n => n.href === pathname)?.label || 'Overview'}
            </span>
          </div>

          <div className="flex items-center gap-6">
            <button className="relative text-[#5a6a7a] hover:text-[#2a85ff] transition-colors cursor-pointer">
              <Bell size={20} />
              <span className="absolute top-0 right-0 w-2 h-2 bg-[#f07830] rounded-full border-2 border-white" />
            </button>
            <div className="h-8 w-px bg-[#e2eaf2]" />
            <Link href="/candidate/profile" className="flex items-center gap-3 group">
              <div className="text-right flex flex-col">
                <span className="text-sm font-bold text-[#070707]">{userName}</span>
                <span className="text-[10px] text-[#8a9ab0] font-semibold uppercase tracking-wider">{completion}% Profile Complete</span>
              </div>
              <div className="w-10 h-10 rounded-full border-2 border-[#e2eaf2] p-0.5 group-hover:border-[#2a85ff] transition-all">
